...
bool is_3way_splittable(std::vector<std::tuple<int,int>>& presents, int target){
    int sum = 0;
    ...
    for(auto& [n, used] : presents){
        ...
        auto res = is_3way_splittable(presents,target);
        used = false;
        if(res){
            return true;
        }
...
std::vector<std::vector<int>> generate_center(std::vector<std::tuple<int,int>>& presents, int target, int starting = 0){
    ...
    for(auto& [n, used] : presents){
        ...
        if(sum + n == target && is_3way_splittable(filtered,target)){
            std::vector<int> v;
